# HNU-CourseWork

## Introduction

The coursework and projects in College of Computer Science and Electronic Engineering (CSEE), Hunan University (HNU).

## Copyrights

The PDF files in this repository is written by their authors (Professors, TAs, Hunan University), all rights reserved. Commercial use is not permitted without permission.

All Python code contained in this repository was written by Rain Chen as part of my experimental assignment for the courses of Hunan University and is not available for commercial use. This part of the code can only be used for reference and communication between students, **it is strictly forbidden to use this code directly as your own experimental lab assignment code to submit**.

## Tips

The experimental content of the courses in HNU **may change** according to the teaching progress and arrangement every year. The contents of this repository are for **learning reference only**.
